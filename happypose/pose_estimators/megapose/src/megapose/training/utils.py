...
def cast_raw_numpy_images_to_tensor(images):
    ...
    if max_rgb < 1.5:
        raise Warning(
            "You are about to divide by 255 but the max rgb pixel value is less than 1.5",
        )
...
class CudaTimer:
    ...
    def elapsed(self) -> float:
        ...
        if not self.start_called:
            raise ValueError(
                "You must call CudaTimer.start() before querying the elapsed time",
            )

        if not self.end_called:
            raise ValueError(
                "You must call CudaTimer.end() before querying the elapsed time",
            )
