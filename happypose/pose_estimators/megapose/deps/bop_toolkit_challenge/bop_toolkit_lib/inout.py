    ...
    with open(path, "w") as f:
        if isinstance(content, dict):
            ...
            for elem_id, (k, v) in enumerate(content_sorted):
                f.write(f'  "{k}": {json.dumps(v, sort_keys=True)}')
            ...
            for elem_id, elem in enumerate(content):
                f.write(f"  {json.dumps(elem, sort_keys=True)}")
                if elem_id != len(content) - 1:
                    f.write(",")
                f.write("\n")
            f.write("]")
...
def load_scene_gt(path):
    ...
    for _im_id, im_gt in scene_gt.items():
        for gt in im_gt:
            ...
            for line in f:
                ...
                if line_id == 1 and header in line:
                    ...
                    result = {
                        ...
                        "score": float(elems[3]),
                        "R": np.array(
                            list(map(float, elems[4].split())),
                            np.float,
                        ).reshape((3, 3)),
                        "t": np.array(
                            list(map(float, elems[5].split())),
                            np.float,
                        ).reshape((3, 1)),
                        "time": float(elems[6]),
                    }
...
def save_ply2(
    ...
    valid_pts_count = 0
    for _pt_id, pt in enumerate(pts):
    ...
    if texture_file is not None:
        f.write(f"comment TextureFile {texture_file}\n")
