...
import numpy as np
from bop_toolkit_lib import config, dataset_params, inout, misc, pose_error, renderer
...
# ------------------------------------------------------------------------------
vsd_deltas_str = ",".join([f"{k}:{v}" for k, v in p["vsd_deltas"].items()])
...
for result_filename in p["result_filenames"]:
    misc.log(f"Processing: {result_filename}")
    ...
    for scene_id, scene_targets in targets_org.items():
        ...
        def save_errors(_error_sign, _scene_errs):
            ...
            misc.log(f"Saving errors to: {errors_path}")
