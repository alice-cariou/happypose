...
from bop_toolkit_lib import config, dataset_params, inout, misc, pose_matching, score
...
p = {
    ...
    "out_matches_tpath": os.path.join(
        "{eval_path}",
        "{error_dir_path}",
        "matches_{score_sign}.json",
    ),
    "out_scores_tpath": os.path.join(
        "{eval_path}",
        "{error_dir_path}",
        "scores_{score_sign}.json",
    ),
}
################################################################################
...
for error_dir_path in p["error_dir_paths"]:
    misc.log(f"Processing: {error_dir_path}")
    ...
    for scene_id, scene_targets in targets_org.items():
        misc.log(f"Processing scene {scene_id} of {dataset}...")
    ...
    misc.log(f"Matching and score calculation took {time_total}s.")
