...
import numpy as np
from bop_toolkit_lib import config, dataset_params, inout, misc
...
misc.log(f"Min dist: {np.min(dists)}")
misc.log(f"Max dist: {np.max(dists)}")
misc.log(f"Mean dist: {np.mean(dists)}")

misc.log(f"Min azimuth: {np.min(azimuths)}")
misc.log(f"Max azimuth: {np.max(azimuths)}")
misc.log(f"Mean azimuth: {np.mean(azimuths)}")

misc.log(f"Min elev: {np.min(elevs)}")
misc.log(f"Max elev: {np.max(elevs)}")
misc.log(f"Mean elev: {np.mean(elevs)}")

misc.log(f"Min visib fract: {np.min(visib_fracts)}")
misc.log(f"Max visib fract: {np.max(visib_fracts)}")
misc.log(f"Mean visib fract: {np.mean(visib_fracts)}")
