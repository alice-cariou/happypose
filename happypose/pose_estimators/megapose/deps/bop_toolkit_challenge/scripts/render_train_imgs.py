...
import cv2
from bop_toolkit_lib import config, dataset_params, inout, misc, renderer, view_sampler
...
for obj_id in obj_ids:
    ...
    inout.save_scene_camera(
        out_scene_camera_tpath.format(out_path=out_path, obj_id=obj_id),
        scene_camera,
    )
    inout.save_scene_gt(
        out_scene_gt_tpath.format(out_path=out_path, obj_id=obj_id),
        scene_gt,
    )
