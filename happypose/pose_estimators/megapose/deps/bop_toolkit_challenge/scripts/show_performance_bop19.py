...
def plot_recall_curves(recall_dict, p):
    ...
    """
    for _i, error in enumerate(p["errors"]):
        if error["type"] == "mspd":
            corr_thres = [f"{e}" for sl in error["correct_th"] for e in sl]
        else:
            corr_thres = [f"{e:.2f}" for sl in error["correct_th"] for e in sl]
        ...
        plt.title(
            error["type"] + " - " + "average recall: " + f"{np.mean(all_recalls):.4f}",
        )
...
for result_filename in p["result_filenames"]:
    misc.log("===========")
    misc.log(f"SHOWING: {result_filename}")
    misc.log("===========")
    ...
    for error in p["errors"]:
        ...
        for error_sign, _error_dir_path in error_dir_paths.items():
    ...
    for err_type in err_types:
        misc.log(f"Average Recall {err_type}: {aur[err_type]}")

    if {"vsd", "mssd", "mspd"}.issubset(err_types):
        test_set = os.path.basename(result_filename)
        mean_error = np.mean([aur[err_type] for err_type in err_types])
        misc.log(f"Average BOP score on {test_set}: {mean_error}")
