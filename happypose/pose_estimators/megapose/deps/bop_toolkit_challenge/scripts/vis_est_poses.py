...
import numpy as np
from bop_toolkit_lib import config, dataset_params, inout, misc, renderer, visualization
            ...
            for ests_vis_id, ests_vis in enumerate(im_ests_vis):
                ...
                if p["vis_per_obj_id"]:
                    vis_name = "{im_id:06d}_{obj_id:06d}".format(
                        im_id=im_id,
                        obj_id=im_ests_vis_obj_ids[ests_vis_id],
                    )
                else:
                    vis_name = f"{im_id:06d}"
